...
variable "api_allow_internet_egress" {
  description = "See modules/security_groups/variables.tf. Temporary escape hatch enabled during the actual staging deployment after an ECR-image-layer-pull-via-VPC-endpoint edge case (manifest/API calls resolved fine through ecr.api/ecr.dkr, but layer blob fetches timed out reaching a public IP) couldn't be resolved within a reasonable troubleshooting window - see docs/aws-production-architecture.md's own \"prove correctness first\" principle. Follow-up: root-cause the VPC-endpoint-only path before using this in real production."
  type        = bool
  default     = false
}

variable "alb_ingress_cidrs" {
  description = "\"Do NOT expose the staging environment publicly until security checks pass\" - set this to your own IP (e.g. [\"203.0.113.4/32\"]) in terraform.tfvars, not the module's own 0.0.0.0/0 default."
  type        = list(string)
  default     = ["0.0.0.0/0"]
}
