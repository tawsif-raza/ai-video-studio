"""
Read-only ffprobe boundary - the postflight companion to a provider's
generate() call. Probes an already-generated clip's actual duration and
stream properties; never writes anything and never judges whether those
properties are acceptable (that's postflight.validate_generated_clip's job,
kept pure and separate).

Deliberately duplicated, not imported, from execution_engine/ffprobe_client.py
(ARCHITECTURE.md SS25.6): reusing that function directly would create a new
cross-engine import edge that doesn't exist anywhere else in the system
(execution_engine and publishing_engine don't import each other either). This
is the small, accepted duplication of a thin ffprobe-invoking wrapper the
architecture note calls for, preserving the "no engine imports another
engine" symmetry."""
...
def probe(path: str, ffprobe_path: str = "ffprobe") -> Optional[ProbedMedia]:
    ...
    timeout/OS error. postflight.validate_generated_clip treats a None probe
    as an unconditional validation failure."""
