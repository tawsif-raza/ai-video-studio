...
| 26 | Video Generation Engine Architecture Design (v2.2.0) | Done | Design-only milestone: architecture, folder structure, contracts, hybrid image/video rendering model, and provider abstraction (Google Veo named as first target, not implemented) for the fifth top-level component, approved ahead of implementation. | `ARCHITECTURE.md` §25 only — no application code |
| 27 | Video Generation Engine Foundation — Stub Provider (this revision, v2.3.0) | Done | Implements §25's design end to end behind a zero-cost local `StubProvider` (no Google Veo, no network calls, no paid quota): contracts, controller, provider registry/abstraction, pre/postflight, and Project Manager persistence (`video_manifest.json`). See §25.17 for the two amendments this implementation pass required. | `shared_core/contracts/video_generation.py`, `video_generation_engine/*`, `project_manager/{manager,project,video_generation_writer}.py`, `tests/test_video_generation_*.py`, `tests/project_manager/test_video_generation_manager.py`, `tests/integration/test_video_generation_execution_integration.py` |
...
| **2.3.0 (this revision)** | **2026-08-07** | **Video Generation Engine Foundation — Stub Provider (Milestone V2).** Implements §25 end to end behind a zero-cost local `StubProvider`: `shared_core/contracts/video_generation.py`, `video_generation_engine/*` (controller, provider registry, request/prompt builders, pre/postflight, errors), and `project_manager` persistence (`video_generation_writer.py`, `get_media_video_dir`, `load_video_generation_manifest`, `save_video_generation_result`, two new `Project` fields — both `Optional`, no migration required). No Google Veo, no network call, no paid quota. A real-ffmpeg integration test proves a stub-generated clip renders alongside an image through the entirely unmodified Execution Engine. Two amendments to §25's original text, both recorded in the new §25.17: `VideoGenerationOptions` gains `fps: int = 30`; `MediaAccessError` is defined locally rather than imported from `execution_engine.errors` (§25.1 rule 1 overrides §25.9's prose, which described a reuse `publishing_engine` never actually implements). No dashboard UI, no budget logic, no account rotation, no breaking API changes. |
...
- **Video Generation Engine** — an opt-in, automated populator of `media/video/` via external AI video providers (Google Veo first), consuming `video_prompts.json` (already produced today) and slotting into the hybrid image/video rendering model Producer Studio and the Execution Engine already implement. Full design (module layout, contracts, provider abstraction, hybrid-mode selection) is recorded in §25 and, as of v2.3.0 (§25.17), implemented behind a zero-cost local `StubProvider`; a real provider (Google Veo), a second provider, dashboard UI, and budget guardrails remain future milestones (§25.15).
...
### 25.17 Milestone V2 Implementation Notes (Stub Provider Foundation)

§25.1–§25.16 are now implemented, not just designed, behind a zero-cost local `StubProvider` — no Google Veo, no network call, no paid quota. `video_generation_engine/` (`controller.py`, `provider_registry.py`, `prompt_builder.py`, `request_builder.py`, `preflight.py`, `postflight.py`, `ffprobe_client.py`, `errors.py`, `providers/{base,stub}.py`), `shared_core/contracts/video_generation.py`, and the `project_manager` additions (`get_media_video_dir`, `load_video_generation_manifest`, `save_video_generation_result`, `video_generation_writer.py`, plus the two new `Project` fields) all exist and are exercised by `tests/test_video_generation_*.py`, `tests/project_manager/test_video_generation_manager.py`, and a real-ffmpeg integration test proving a stub-generated clip renders alongside an image through the entirely unmodified Execution Engine. Following this document's own rule (§2: a difference from the design is proposed as an amendment, not made silently), implementation surfaced two amendments:

1. **`VideoGenerationOptions` gains `fps: int = 30`**, mirroring `RenderOptions.fps`. §25.4's original table didn't anticipate this because no provider existed yet to need a concrete frame rate — the first real implementation (even a local one) does.
2. **`MediaAccessError` is defined locally in `video_generation_engine/errors.py`, not imported from `execution_engine.errors`.** §25.9's text described reusing it "the same reuse `publishing_engine`'s design already makes (§24.7)" — checked against the actual repository during implementation, `publishing_engine/errors.py` never implements that reuse (it only reserves the name in a comment; no such import exists there). Importing `execution_engine.errors.MediaAccessError` here would have satisfied §25.9's prose but broken §25.1 rule 1's stricter, structurally-verified boundary ("never imports `execution_engine`") — confirmed by grepping `video_generation_engine/` for any `execution_engine`/`publishing_engine`/`director_studio`/`producer_studio`/`agents` import (zero matches). Rule 1 wins.

One further, deliberate deviation from the *milestone brief's* (not this document's) example file list: the brief lists a `contracts.py` inside `video_generation_engine/`. Per §25.6's existing note, contracts live in `shared_core/contracts/video_generation.py` instead — no `video_generation_engine/contracts.py` was created, consistent with the Publishing Engine precedent this document already established.

**Explicitly still not built** (unchanged from §25.15, all deferred to V3+): Google Veo's real request/response shape and auth flow, a second real provider, the dashboard UI (§25.11), budget/quota guardrails, the cleanup policy for superseded/failed clips (§25.14), the memory-profiling exercise, and a CLI entry point (§25.16 item 5 — this pass exercises the engine only via its Python API and tests, not yet a `video_app.py`-style command).
