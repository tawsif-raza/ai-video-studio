...
- You decide how many shots each scene needs — usually 1 to 3. A short, simple scene
  (under ~5s) often needs just 1 shot. A longer or emotionally complex scene can use
  2-4 shots to build the moment (e.g. a wide establishing shot, then a close-up reaction).
- Every scene_id from the Production Plan must appear exactly once in your output —
  do not skip scenes, do not invent new ones.
- Shot durations within a scene must add up to approximately that scene's target duration.
- Only reference characters that are already listed as present in that specific scene —
  never introduce a character a shot that scene didn't already include.
